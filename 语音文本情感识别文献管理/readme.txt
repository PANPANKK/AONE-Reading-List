...
###Anand23Multi-label（有代码，可以重点关注下）
然而，情感状态的共现（例如，“愤怒”和“沮丧”可能同时出现或相互影响）及其动态演变可能由于说话者的内部（例如，其个性化的社会文化教育和人口背景的影响）和外部情境而显著变化。
到目前为止，以往的研究主要集中在评估在给定时间观察到的主导情感，这在测试期间对于难以处理的多标签情感可能会产生误导性的分类决策。
在这项工作中，我们提出了一种通过高效的多模态Transformer网络进行的自监督多标签同伴协同蒸馏（SeMuL-PCD）学习方法，其中来自多个特定模式的同伴网络（例如，文本、音频、视觉）的互补反馈被蒸馏到一个单一的模式融合网络中，以同时估计多种情感。
所提出的多模态蒸馏损失通过最小化与同伴网络的Kullback-Leibler散度来校准融合网络。此外，每个同伴网络通过自监督对比目标进行条件处理，以提高在不同社会人口背景下的泛化能力。
通过启用允许每个网络独立学习其模式特定辨别模式的同伴协同学习，SeMuL-PCD在不同的会话环境中表现出色。特别是，该模型不仅在多个大规模公共数据集（例如，MOSEI、EmoReact和ElderReact）上超过了当前的最先进模型，而且在跨数据集实验设置中提高了大约17%的加权F1分数。该模型还展示了在不同年龄和人口背景的群体中令人印象深刻的泛化能力。

### Fayet18EMO&LY
根据（Chandola 等，2009）的定义，“异常被定义为一种不符合预期正常行为的模式”。异常检测系统已应用于多个领域：入侵检测（Axelsson，2000）、欺诈检测（Abdallah 等，2016）、传感器网络（Park 等，2010）、飞行安全监控（Li 等，2011）或视频监控（Ko，2008）。
人类异常检测工作中常使用或展示的语料库中，通常没有提供受试者的语音和/或面部。然而，异常检测范围内可用的语音语料库主要集中在疾病、压力（Hansen 等，1997）和抑郁症的检测。例如，NKI-CCRT 语料库（Clapham 等，2012）被构建用于研究癌症前后语音清晰度。在（Giraud 等，2013）中，展示了一个包含公众演讲任务期间多模态压力表达的语料库。
虽然该语料库不是专门用于异常检测，但它具有多模态的优势，特别是包括参与者的语音和面部表情。这样的语料库使得可以联合研究面部表情和语音。
在情感方面，也有一些语料库可用，如贝尔法斯特自然数据库（Douglas-Cowie 等，2003），包含人们讨论情感主题的记录，或 EmoTV1 语料库（Abrilian 等，2005），包含自然的、非表演的情感标记数据的电视采访。这些语料库都是为了回答特定的科学问题而设计和收集的。
据我们所知，没有包含受控、表演或自然异常的情感视听语料库，专门解决异常检测问题。
因此，在本文中，我们介绍了一个新的互补多媒体语料库，名为EMOLY，包含以人为中心的异常。该语料库由41名受试者组成，这些受试者阅读一个故事三次。在阅读过程中，我们诱导受试者产生一种可以视为在特定情境下的异常行为的反应。
该语料库收集了在强度、模态（语音或面部表情）或情感方面各种异常反应。本文余下的部分结构如下：第2节详细描述了语料库的设计，第4节描述了语料库的内容，最后，第5节介绍了使用该语料库的初步实验


###Wu23Context-Constrained (代码地址：https://github.com/crowpeter/RealDeception)
审讯由证据驱动的对话组成，需要适当整合上下文，而大多数先前的工作将其视为一个序列建模任务。在这项工作中，我们提出了一种上下文约束的句子建模方法来检测欺骗。
具体来说，我们引入了一个在多句子上定义的全局上下文标签，即如果其任何句子具有欺骗性，则该上下文标签被标记为欺骗。
然后，通过使用一个上下文整合器聚合局部句子的预测以进行上下文标签预测，我们通过联合优化全局和局部标签来提高欺骗检测。
我们使用Opensmile提取Emobase作为我们的声学特征 [17]，这在最近的一项欺骗研究中也被使用 [12]。Emobase包含声学-韵律属性（MFCC、基本频率(F0)和语音质量包络等）及其统计函数，结果每个话语有988维特征。
我们使用双向编码器表示（BERT）作为我们的词嵌入特征,使用CKIP工具 [19]进行分词，然后输入预训练的bert-base-chinese模型。
会话时间动态（CTD）特征:
持续时间(d): 代理人提问轮或应答者回答轮的总持续时间。
语速: 通过Nwords/d计算一个问题或回答的说话速度。
静默时间(ds): 上一个话语结束时间点与当前话语开始时间点之间的持续时间。
静默话语比率: 一个话语中的d与ds的比率，计算方法为(ds/d)。
持续时间差异: 当前话语持续时间与上一个话语持续时间之间的差异(d - dlast)。
持续时间相加: 当前话语持续时间与上一个话语持续时间之和(d + dlast)。
持续时间比率: 当前话语持续时间与上一个话语持续时间之间的比率(d/dlast)。
我们的方法显著优于其他模型，在局部和全局水平上的无权平均召回率（UAR）分别达到76.38%和73.15%。我们还进行了两项分析，进一步证明了我们方法的有效性。

###Chou21Automatic
在这项研究中，我们提出了一个自动欺骗检测框架，通过全面整合领域内对欺骗行为理解的先验知识。具体而言，我们计算了声学特征、文本信息、与非语言行为的隐含关系以及会话时间动态，以改进对话中的自动欺骗检测。
所提出的模型在普通话每日欺骗对话语料库（DDDM）数据库上达到了最先进的性能，在欺骗识别中的无权平均准确率为80.61%。
在进一步的分析中，我们揭示了(i)在会话时间动态特征中可以通过询问者的行为观察到欺骗者的欺骗行为，(ii)一些声学特征（例如响度和MFCC）和文本特征是检测欺骗行为的重要且有效的指标。

###Cat22Speaker
获得大型的语音情感识别数据集是很困难的，而小型数据集可能包含偏见。这些偏见通常会损害模型的泛化能力。为了解决这一挑战，我们提出了一种基于梯度的对抗学习框架，该框架在学习语音情感识别任务的同时，从特征表示中归一化说话者特征。
我们在说话者独立和说话者依赖的环境中展示了我们方法的有效性，并在具有挑战性的IEMOCAP数据集上取得了新的最先进结果。
